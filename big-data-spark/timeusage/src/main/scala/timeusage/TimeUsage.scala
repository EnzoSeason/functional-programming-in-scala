...
object TimeUsage extends TimeUsageInterface :
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsIds = List("t01", "t03", "t11", "t1801", "t1803")
    val workingIds = List("t05", "t1805")
    val otherIds = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val primaryNeedsColumns = for {
      name <- columnNames
      id <- primaryNeedsIds
    ...
    val otherColumns = for {
      name <- columnNames
      if !primaryNeedsColumns.contains(col(name)) && !workColumns.contains(col(name))
      id <- otherIds
      if name.startsWith(id)
    } yield col(name)
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] =
    ...
    def round1(d: Double): Double = (d * 10.0).round / 10.0
