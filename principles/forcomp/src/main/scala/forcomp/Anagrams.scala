...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = w
    .groupBy((c: Char) => c.toLower)
    .map((c: Char, grouped: String) => (c, grouped.length))
    .toList
    .sortBy((c: Char, _) => c)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.foldRight(List[Occurrences](Nil))(
      (curr: (Char, Int), acc: List[Occurrences]) => {
        val (c, count) = curr
        val comb: List[Occurrences] = for {
          prevComb <- acc
          i <- 1 to count
        } yield (c, i) :: prevComb
        acc ++ comb
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    // transform y, from List to Map
    val yMap = y.foldLeft(Map[Char, Int]())(
      (acc: Map[Char, Int], curr: (Char, Int)) => {
        val (c: Char, count: Int) = curr
        acc + (c -> count)
      })

    x.map((c: Char, count: Int) => (c, count - yMap.getOrElse(c, 0)))
