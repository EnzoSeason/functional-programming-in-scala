...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = Tweet("dummy", "dummy", -1)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    ...
    val selfMost = if (leftMost.retweets > rightMost.retweets) leftMost else rightMost
    if (selfMost.retweets > elem.retweets) selfMost else elem
  }

  def descendingByRetweet: TweetList = {
    val selfMost = mostRetweeted
    Cons(selfMost, remove(selfMost).descendingByRetweet)
  }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists(txt => tw.text.contains(txt)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists(txt => tw.text.contains(txt)))
